...
class socialize_this {
	...
	public function __construct(){
		...
		if(get_option('st_css_sheet') == TRUE){
			wp_enqueue_style('SocializeThis', WP_PLUGIN_URL . '/socialize-this/st.css', false);
		}
...
<a href="?page=socialize-this&module=social_widgets"><?php _e('Social Widgets', 'st_plugin'); ?></a> (<a href="?page=socialize-this&module=tagList"><?php _e('Tag List', 'st_plugin'); ?></a>) | 
...
<em>
	...
	<tr>
        ...
        <td width="60%"><label>
            ...
            <option value="isgd"<?php if(get_option('st_url_shortening_service') == 'isgd'  || get_option('st_url_shortening_service') == ''){echo ' selected="selected"';}?>>is.gd</option>
            <option value="trim"<?php if(get_option('st_url_shortening_service') == 'trim'){echo ' selected="selected"';}?>>tr.im</option>
	...
	if(isset($_POST['add_widget'])){
		...
		$count = 0;
		if(is_array($_POST['set_file'])){foreach($_POST['set_file'] as $set_file){
	...
	<tr id="<?php echo $widget['ID']; ?>" <?php if($widget['enabled'] == '2'){echo 'class="enabled"';}else{echo 'class="disabled"';} ?> >
		...
		<td><input class="updateEnabled" title="<?php echo $widget['ID']; ?>" name="enabled[<?php echo $widget['ID']; ?>]" type="checkbox" value="2" <?php if($widget['enabled'] == '2'){echo 'checked';} ?> /></td>
	...
	foreach($this->st_get_widget_sets() as $set_file){
		...
		if(is_object($widget_set)){
			?>
			<tr>
			<td><?php echo $widget_set->name;
			if($widget_set->description != ''){echo ' - '.$widget_set->description;}
			if($widget_set->preview != ''){?><br />
	...
	private function st_adm_advanced_functions(){
		if(isset($_POST['save_advanced_settings'])){
			...
			if($_POST['st_css_sheet'] == 'yes'){update_option('st_css_sheet', FALSE, TRUE);} else {update_option('st_css_sheet', TRUE, TRUE);}
			if($_POST['st_cache'] == 'yes'){update_option('st_cache', TRUE, TRUE);} else {update_option('st_cache', FALSE, TRUE);}
			if($_POST['st_menu_page'] == 'yes'){update_option('st_menu_page', TRUE, TRUE);} else {update_option('st_menu_page', FALSE, TRUE);}
  ...
  <?php
	...
	public function socialize_this_settings(){
		...
		if(isset($_GET['module'])){
			switch($_GET['module']){
				...
				case 'edit_widget': $this->st_adm_edit_widget(); break;
				...
				case 'settings': $this->st_adm_settings(); break;
				case 'tagList': $this->st_adm_tagList(); break;
	...
	private function shorten_url($url){
		...
		elseif(get_option('st_url_shortening_service') == 'trim'){return $this->get_link('http://api.tr.im/v1/trim_simple?url='.urlencode($url));}
		...
		elseif(get_option('st_url_shortening_service') == 'bitly'){return $this->bitly->shorten($url);}
		else{return $this->get_link('http://is.gd/api.php?longurl='.urlencode($url));}
		return $url;
	}
	...
	public function show_social_widgets($names=NULL, $post_id=NULL, $permalink=NULL, $title=NULL){
		...
		$vars = apply_filters('st-tags', $vars);
		if(is_object($post)){
			if(get_post_meta($post->ID, 'st_hide_icons', TRUE) == TRUE){ return FALSE; }
	...
	private function getWidgets($enabled=NULL, $name=NULL, $id=NULL, $getHTML=null){
		...
		if($enabled == 2 || $enabled == 1){ $where[] = "`enabled` = ".$enabled."";}
		if($name != NULL){
			if(is_array($name)){
	...
	private function updateWidget($id, $html=NULL, $position=NULL, $enabled=2){
		...
		if($position != NULL){$update['position'] = (int) $position;}
	...
	public function st_install($upgrade=NULL){
		...
		if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
			if($upgrade == NULL){ // If it's a fresh install.
				add_option('st_template_twitter_noti', '%raw_title% %raw_short_url%');
